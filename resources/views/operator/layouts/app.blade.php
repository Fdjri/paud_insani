...
<head>
    ...
    <title>Operator - PAUD Insani</title>
...
<body class="bg-gray-100">
        ...
        <div id="sidebar" class="bg-white shadow-lg w-64 shrink-0 fixed inset-y-0 left-0 lg:relative lg:translate-x-0 transform -translate-x-full transition-all duration-300 ease-in-out z-30">
            <div class="flex flex-col h-full">
                ...
                <nav class="flex-1 px-4 py-2 space-y-1">
                    <a href="{{ route('operator.dashboard') }}" class="flex items-center justify-center lg:justify-start px-4 py-2.5 rounded-lg transition-colors duration-200 {{ request()->is('operator/dashboard*') ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100' }}">
                    ...
                    <a href="{{ route('operator.siswa.index') }}" class="flex items-center justify-center lg:justify-start px-4 py-2.5 rounded-lg transition-colors duration-200 {{ request()->is('operator/data-siswa*') ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100' }}">
                        <i class="las la-users text-xl w-5 h-5 shrink-0"></i>
                        <span class="ml-3 font-medium sidebar-text">Data Siswa</span>
                    </a>

                    <a href="{{ route('operator.guru.index') }}" class="flex items-center justify-center lg:justify-start px-4 py-2.5 rounded-lg transition-colors duration-200 {{ request()->is('operator/guru*') ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100' }}">
            ...
            <header class="flex justify-between items-center p-4 bg-white border-b shadow-sm">
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-xl z-20" style="display: none;">
                        <div class="py-2">
                            <a href="{{ route('operator.profile.index') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
    ...
    <script>
        // Listener untuk interaksi DOM dasar seperti sidebar
        ...
        // Listener untuk interaksi yang bergantung pada Livewire
        document.addEventListener('livewire:init', () => {
            ...
            window.addEventListener('show-delete-confirmation', event => {
                Swal.fire({
                    ...
                    text: `Data bernama "${event.detail.name}" tidak bisa dikembalikan.`,
                    icon: 'warning',
